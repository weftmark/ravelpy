...
@pytest.mark.asyncio
async def test_none_params_stripped(client, mock_api):
...
@pytest.mark.asyncio
async def test_basic_auth_sent(client, mock_api):
...
@pytest.mark.asyncio
async def test_raises_ravelry_api_error_on_401(client, mock_api):
    mock_api.get("/current_user.json").respond(401, text="Unauthorized")
    with pytest.raises(RavelryAPIError) as exc_info:
        await client.people.me()
    assert exc_info.value.status_code == 401
    assert "Unauthorized" in exc_info.value.message


@pytest.mark.asyncio
async def test_raises_ravelry_api_error_on_404(client, mock_api):
    mock_api.get("/patterns/999999.json").respond(404, text="Not Found")
    with pytest.raises(RavelryAPIError) as exc_info:
        await client.patterns.show(pattern_id=999999)
    assert exc_info.value.status_code == 404


@pytest.mark.asyncio
async def test_raises_ravelry_api_error_on_500(client, mock_api):
    mock_api.get("/yarns/1.json").respond(500, text="Internal Server Error")
    with pytest.raises(RavelryAPIError) as exc_info:
        await client.yarns.show(yarn_id=1)
    assert exc_info.value.status_code == 500


@pytest.mark.asyncio
async def test_returns_parsed_json(client, mock_api):
    payload = {"pattern": {"id": 42, "name": "Test Pattern"}}
    mock_api.get("/patterns/42.json").respond(200, json=payload)
    _data, _etag, _raw = await client.patterns.show(pattern_id=42)
    assert _raw == payload
